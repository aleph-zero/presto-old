...
import io.bunnysoft.presto.metadata.MetadataUtils;
import io.bunnysoft.presto.metadata.ShardMetadata;
import io.bunnysoft.presto.types.TypeUtils;
import io.bunnysoft.presto.util.Types;
...
import static io.bunnysoft.presto.util.DateUtil.determineDateType;
...
public class ElasticsearchMetadata implements ConnectorMetadata
{
    ...
    public void createTable(ConnectorSession session, ConnectorTableMetadata tableMetadata)
    {
        ...
        if (MetadataUtils.tableExists(client, schema.getSchemaName(), schema.getTableName())) {
            throw new PrestoException(ALREADY_EXISTS, format("Table [%s.%s] already exists", schema.getSchemaName(), schema.getTableName()));
        }

        OptionalInt shards   = ElasticsearchTableProperties.getNumberOfShards(tableMetadata.getProperties());
        OptionalInt replicas = ElasticsearchTableProperties.getNumberOfReplicas(tableMetadata.getProperties());
        ...
        try {
            mapping = MetadataUtils.createMapping(schema.getTableName(), columns);
        }
        catch (IOException e) {
        ...
        try {
            ...
            client.admin().indices().prepareCreate(schema.getSchemaName())
                    ...
                    .setSettings(MetadataUtils.createIndexSettings(shards, replicas))
                    .execute().get();
        }
    ...
    {
        ElasticsearchTableHandle handle = Types.checkType(table, ElasticsearchTableHandle.class, "table");
    ...
    {
        ElasticsearchTableLayoutHandle layout = Types.checkType(handle, ElasticsearchTableLayoutHandle.class, "handle");
    ...
    {
        ElasticsearchTableHandle handle = Types.checkType(tableHandle, ElasticsearchTableHandle.class, "tableHandle");
    ...
    {
        Types.checkType(tableHandle, ElasticsearchTableHandle.class, "tableHandle");
        return Types.checkType(columnHandle, ElasticsearchColumnHandle.class, "columnHandle").toColumnMetadata();
    }
        ...
        for (Map.Entry<String, Object> entry : ((Map<String, Object>) mappings.get("properties")).entrySet()) {
            ...
            if (type == null || type.isEmpty()) {
                ...
                if (type.equals("date")) {
                    ...
                    builder.add(new ColumnMetadata(entry.getKey(), array ? new ArrayType(TypeUtils.type(type)) : TypeUtils.type(type)));
                }
            }
        }
